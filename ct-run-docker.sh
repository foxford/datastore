...
CLEAN \
	&& cd deps/riakacl && \
		DOCKER_RUN_OPTIONS="-dt --name ${DOCKER_RIAKACL_CONTAINER_NAME}" \
	...
	&& cd - && \
		...
		DOCKER_CONTAINER_COMMAND="touch ${DOCKER_EVENT_DONE} && riak attach" \
		./run-docker.sh \
	&& while [[ ! -f "deps/riakacl/${DOCKER_EVENT_DONE}" || ! -f "${DOCKER_EVENT_DONE}" ]]; do sleep 3; done \
